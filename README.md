...
**Windows (Git Bash):** замість `python3` — `python`, активація —
`source venv/Scripts/activate`. Якщо `python --version` друкує лише
`Python` без номера версії — це заглушка Microsoft Store, а не
інтерпретатор: встанови Python з python.org (галочка "Add python.exe to
PATH") і вимкни в Параметрах Windows → Програми → Додаткові параметри
програм → "Псевдоніми виконання програм" пункти `python.exe`/`python3.exe`.
`ANTHROPIC_API_KEY` — платний ключ з console.anthropic.com (підписка
Claude.ai його не покриває).
...
Усі тести (`tests/`, 46 штук) мокають Google Drive/Docs/Sheets і виклик
...
`TRACKER_SHEET_TITLE` (таблиця відгуків) — свідомо БЕЗ суфікса: це твоя
ручна таблиця без тестової копії. Пайплайн читає з неї URL (Рівень 2
дедублікації) і точково дописує статуси за листами (Крок 3, див. "Відомі
обмеження", п. 5) — тож працює завжди з реальним файлом.
...
- **Windows Task Scheduler** (рекомендовано для Windows): один раз з кореня
  репо — `powershell -ExecutionPolicy Bypass -File scripts\register_windows_task.ps1`
  (за замовчуванням щодня о 12:00 місцевого часу, тобто ПІСЛЯ автозапуску
  чат-версії, щоб Крок 5 мав з чим порівнювати; інший час — `-Time 12:30`).
  Якщо о цій порі комп'ютер спав — задача запуститься, щойно він прокинеться
  (`StartWhenAvailable`). Вивід — у `logs/scheduler.log`, звіт — як завжди
  в `reports/`.
...
## HTTP 403 від robota.ua / work.ua / freelancehunt.com

Ці сайти відсікають звичайний `requests` за TLS/HTTP2-відбитком — навіть із
браузерним User-Agent (підтверджено запуском 25.09.2026: 403 стабільно, не
разово). Тому їхні скрапери беруть сесію `get_session(impersonate=True)`
(`scrapers/base.py`): вона йде через `curl_cffi`, який відтворює справжній
відбиток Chrome. djinni/dou/happymonday/Telegram і далі на звичайному
`requests` — там 403 немає. Якщо `curl_cffi` не встановлено — фолбек на
`requests` з попередженням у лог.

Freelancehunt додатково має **офіційне API v2** — основний шлях Кроку 1.2,
якщо в `.env` задано `FREELANCEHUNT_API_TOKEN` (freelancehunt.com →
Налаштування профілю → API): стабільний JSON замість парсингу HTML. Без
токена — HTML-скрапінг категорій через `curl_cffi`.
...
5. **"Крок 3 (додатковий блок)" — оновлення таблиці "Ворошилов відгуки на
   вакансії" — реалізовано через пошту, а не через пам'ять чату**
   (`pipeline/step3_tracker.py`). Чат-версія брала факти з memory-файлів
   Claude, недоступних автономному скрипту; Python-версія бере їх із листів,
   які Крок 2 класифікував як "відмова" або "запрошення на співбесіду", і
   оновлює рядок ЛИШЕ коли лист однозначно належить одній компанії з
   таблиці: відмова → "Результат відгуку" = відмова + примітка; запрошення →
   лише примітка. Кілька рядків однієї компанії без явної вказівки на
   вакансію (як 2 вакансії Plamigo) — пропуск із поясненням у Кроці 4.
   Ідемпотентно (тема листа в примітках = вже записано), вимикається
   `config.TRACKER_AUTO_UPDATE = False`. Факти, яких немає в пошті (подав
   заявку, пройшов співбесіду офлайн), як і раніше — вручну або в чаті.
   Попутно виправлено: заголовки таблиці лежать у рядку 2 (рядок 1 —
   об'єднана шапка), а Рівень 2 дедублікації раніше шукав їх лише в рядку 1
   і мовчки не працював — тепер `sheets.find_header_row` шукає в перших 5.
...
8. **Таблиця "Ворошилов заявки на фріланс" — manual-only**: пайплайн лише показує
